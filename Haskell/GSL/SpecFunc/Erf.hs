...
erfseries :: Double -> Result Double Double
erfseries x = go 1 x x undefined
    where
        go k coef e del
            | k < 30    = 
                let
            ...
            | otherwise = 
                let
                    val = 2 / sqrt pi * e
                    err = 2 / sqrt pi * (abs del + eps)
                in Result val err

erf :: Double -> Double
erf = val . erf_e

erf_e :: Double -> Result Double Double
erf_e x
    | abs x < 1 = erfseries x
    | otherwise = case erfc_e x of
        Result v e -> Result (1 - v) (e + 2 * eps * abs v)

erfc :: Double -> Double
erfc = val . erfc_e

erfc_e :: Double -> Result Double Double
erfc_e x = wrap estimate
    where
        ax = abs x
        estimate
            | ax <= 1   = cheb_eval_e erfc_xlt1_cs (2 * ax - 1)
            | ax <= 5   = 
                let ex2 = exp (negate x*x)
                    t = 0.5 * (ax - 3)
                    Result cval cerr = cheb_eval_e erfc_x15_cs t
                in Result (ex2 * cval) (ex2 * (cerr + 2 * abs x * eps))
            | ax < 10   = 
                let exterm = exp (negate x * x) / ax
                    t = (2 * ax - 15) / 5
        ...
        wrap (Result v e)
            ...
            | otherwise = Result v       (e + 2 * eps * abs v)
